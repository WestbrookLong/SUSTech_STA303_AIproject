...
class DQNSolver:
    ...
    def experience_replay(self):
        ...
        s_t  = torch.as_tensor(s,  dtype=torch.float32, device=self.device)               # [B, obs_dim]   #these are data batch from buffer
        ...
        with torch.no_grad():
            q_next = self.target(s2_t).max(dim=1, keepdim=True)[0]  # [B, 1] = max_a' Q_target(s',a')    #this is a max of sample, therefore take it as target
            target = r_t + m_t * self.cfg.gamma * q_next            # [B, 1]  this is the critical step of Q-learning
    ...
    def load(self, path: str):
        ckpt = torch.load(path, map_location=self.device,weights_only=False)  # 建议先去掉 weights_only
        ...
        # 恢复 steps / epsilon，如果没有就用默认值
        self.steps = ckpt.get("steps", 0)
        # self.exploration_rate = ckpt.get(["exploration_rate"])
