"""
Behavior Cloning (BC) trainer for offline imitation.
"""
...
class BCConfig:
    ...
    seed: int = 42
    log_dir: str = os.path.join(RUNS_DIR, "bc")
...
class BCTrainer:
    def __init__(self, cfg: BCConfig):
        ...
        self.ckpt_path = os.path.join(self.run_dir, "bc_policy.pt")
        with open(os.path.join(self.run_dir, "run_config.json"), "w") as f:
            json.dump({"algorithm": "bc", **asdict(cfg), "dataset_metadata": self.dataset.metadata}, f, indent=2)

    def _bc_loss(self, batch) -> torch.Tensor:
        ...
        if self.is_discrete:
            ...
            std = log_std.exp()
            # Assume actions are already in the correct scale
    ...
    def save(self, path: str):
        ...
        print(f"[BC] Saved policy to {path}")


def load_policy(path: str, device: str | None = None):
    """Load a BC policy for evaluation."""
